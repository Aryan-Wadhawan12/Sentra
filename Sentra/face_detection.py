...
# Prepare CSV log files (with portable paths)
csv_file = resource_path('stranger1_log.csv')
recognition_log_file = resource_path('recognition_log.csv')
            ...
            for _, row in results[0].iterrows():
                if row['distance'] < 0.3:
                    ...
                    if identity not in marked_today:
                        ...
                        with open(recognition_log_file, 'a', newline='') as f:
...
print("Execution complete. Press Enter to close...")
input()
